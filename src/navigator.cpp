		...
		for(int round = 0; round < maxSearchingAttempts; round++) {
			...
			for (int i = 0; i < lenght; ++i) {
				if(!(isnan(currentPoints[i].x) || isinf(currentPoints[i].x) ||
					...
					// Is transition in threshold
					if(fabs(points[i].z - result.at<float>(0,0)) < compareThreshold &&
					   fabs(points[i].x - result.at<float>(1,0)) < compareThreshold) {
						positivePoints++;
					}
				}
			}
...
void featureCallback(const stroll_bearnav::FeatureArray::ConstPtr &msg)
{
	if (state == NAVIGATING)
	{
		...
		if (count < minGoodFeatures)
			differenceRot = 0;
		for (int i = 0; i < numBins; i++)
			feedback.histogram.push_back(histogram[i]);
		...
		if (isRating)
		{
			...
			if (numFeatureRemove > mapFeatures.feature.size())
				numFeatureAdd = numFeatureRemove = mapFeatures.feature.size();
		...
		if (image_pub_.getNumSubscribers() > 0)
		{
			...
			if (currentImage.rows > 0 && mapKeypoints.size() > 0 && currentKeypoints.size() > 0)
			{
				...
				if (showAllMatches)
				{
					...
					if (showGoodMatches)
						drawMatches(mapIm, kpMap, curIm, kpCur, best_matches, outtran, Scalar(0, 255, 0), Scalar(0, 255, 0), vector<char>(), 3);
				}
				else
				{
					if (showGoodMatches)
						drawMatches(mapIm, kpMap, curIm, kpCur, best_matches, outtran, Scalar(0, 255, 0), Scalar(0, 255, 0), vector<char>(), 2);
				}
				output = outtran.t();
...
void distanceCallback(const std_msgs::Float32::ConstPtr &msg)
{
	...
	if (state == NAVIGATING)
	{
		...
		if (path.size() > currentPathElement)
		{
			...
			if (fabs(path[currentPathElement].angular) > 0.001)
				velocityGain = 1.0;
	...
	{
		if (path.size() > 0)
			overshoot = msg->data - path[path.size() - 1].distance;
		twist.linear.x = twist.linear.y = twist.linear.z = 0.0;
